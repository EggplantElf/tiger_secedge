...
class Token(object):
    ...
    def __init__( self, line ):
        ...
        if entries[12] == '_':
            ...
            self.sec_heads = zip(entries[12].split('|'), entries[14].split('|'))
        self.sec_subj = {}
        self.deps = []
...
def postprocess_tokens(sentence):
    for token in sentence:
        ...
        for (index, label) in token.sec_heads:
            if label in ['SBM', 'SBA', 'SBR', 'SBE', 'SBC']:
...
if __name__=='__main__':
    ...
    if args.train:
        ...
        # for k in feattab.labeltab:
        #   print k, feattab.labeltab[k]
